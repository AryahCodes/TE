...
def circuit(x, theta):
    ...
    for i in range(N_QUBITS):
        qml.RY(i*torch.arccos(x), wires=i)
...
def model(x, theta):
    ...
    return qnode(x_rescaled, theta)

def loss_diff_fnc(theta):
...
def loss_boundary_fnc(theta):
    x = torch.linspace(0.0, 1.0, 100, requires_grad=True, device=device)
    u_0 = model(torch.zeros_like(x), theta)
    return torch.mean((u_0 - 0.75)**2)

def loss_fnc(theta):
    loss_diff     = loss_diff_fnc(theta)
    loss_boundary = loss_boundary_fnc(theta)
...
def run_lbfgs_opt(n=200):
    ...
    for i in range(n):
        opt.step(closure)
        current_loss = loss_fnc(theta).item()
...
for i in range(1):
    ...
    loss_history_COBYLA = run_cobyla_opt(n=5000)
...
print("idx lbfgs_1 lbfgs_2 lbfgs_3 lbfgs_4 lbfgs_5 lbfgs_mean")
for i in range(len(lbfgs_data[0])):
    print(f"{i} {lbfgs_data[0][i]} {lbfgs_data[1][i]} {lbfgs_data[2][i]} {lbfgs_data[3][i]} {lbfgs_data[4][i]} {lbfgs_mean[i]}")
# %%
print("idx cobyla_1 cobyla_2 cobyla_3 cobyla_4 cobyla_5 cobyla_mean")
for i in range(len(cobyla_data[0])):
    print(f"{i} {cobyla_data[0][i]} {cobyla_data[1][i]} {cobyla_data[2][i]} {cobyla_data[3][i]} {cobyla_data[4][i]} {cobyla_mean[i]}")
# %%
print("idx adam_1 adam_2 adam_3 adam_4 adam_5 adam_mean")
for i in range(len(adam_data[0])):
    print(f"{i} {adam_data[0][i]} {adam_data[1][i]} {adam_data[2][i]} {adam_data[3][i]} {adam_data[4][i]} {adam_mean[i]}")
# %%
