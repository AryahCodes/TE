...
x = torch.linspace(0.0, X_END, X_COLOC_POINTS, device=device, requires_grad=True)
...
def compute_MSE_reference():
    if FNN_BASIS:
        ...
        u = eval_circuit(x)
    return torch.mean((u - reference_solution)**2)
...
def loss_fnc():
    ...
    res_pde = du_dx - (4*u - 6*u**2 + torch.sin(50*x) + u*torch.cos(25*x) - 0.5)
    return BOUNDARY_SCALE*torch.mean((u[0] - 0.75)**2) + torch.mean(res_pde**2)
...
for N_LAYERS in N_LAYERS_values:
    ...
    for run in range(5):
        ...
        FNN_BASIS = True
        theta  = torch.tensor(2*torch.pi*torch.rand(N_LAYERS, N_WIRES, 3),requires_grad=True, device=device)
        basisNet = FNNBasisNet(N_LAYERS, BRANCH_WIDTH).to(device)
...
ax.plot(N_LAYERS_values, np.mean(mse_ref_values, axis=1), label="No Embedding (MSE)", marker="o", c="r", linestyle="--")
ax.plot(N_LAYERS_values, np.mean(mse_ref_values_basis, axis=1), label="With Embedding (MSE)", marker="o", c="b", linestyle="--")
